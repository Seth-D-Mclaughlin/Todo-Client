...
const TodoList = (props) => {
    const deleteTodo = (todo) => {
        fetch(`${APIURL}/task/${todo.id}`, { /// we can't do a get min becuase the get fetch doesn't work but we still need to make it match the user only not a get all
            method: 'DELETE',
            headers: {
    ...
    return props.todos.map((todo, index) => {
        return(
            <div class="list-group" key={index}>
                <a href="#" class="list-group-item list-group-item-action list-group-item-secondary flex-grow 0">
